...
function wfKind(p,k,label){
  ...
  const weeks=remIdx(p).map(i=>{ const tot=p[k+'_wt'][i];
    return `<div class="wk-in"><label>${i+1}주</label><input type="number" min="0" max="${tot}" value="${WFh[k][i]??0}" oninput="setWk('${k}',${i},this,${tot})"><span class="wk-tot">/${tot}h</span></div>`; }).join('');
...
function setWk(k,i,el,max){ // 배정시간(max) 초과·음수 입력은 입력칸 값까지 되돌려 막는다
...
function confirmAction(kind){
  ...
  // 연락 완료: 오늘 이미 연락 기록(캘린더 추가 등)이 있으면 중복 로그를 만들지 않고 상태만 변경
  const alreadyToday = kind==='contacted' && (p.contacts||[]).some(c=>c.d===DEMO_DAY);
  if(!alreadyToday){
    const cid=++CID; // 연락/처리 기록 ↔ 처리 내역 연결 (삭제 동기화)
    p.contacts.push({d:DEMO_DAY, memo:label, cid});
    LOGS.unshift({ t:nowStr(), id:p.id, nm:p.nm, type: isCall?'직접연락':(p.doc==='미비'?'서류안내':'출석독려'),
      ai:g.msg, final:(document.getElementById('ai-msg')||{}).value||g.msg, result:kind, reason:'', summary:g.summary, cid });
  }
  toast('✅ '+label+' · 기록됨');
...
function renderLogs(){
  ...
  v.innerHTML=`<div class="sec-h"><h2>처리 내역 · ${LOGS.length}건</h2><div class="hint">연락·정산확정 처리 이력</div></div>
   <table><thead><tr><th>시각</th><th>참여자</th><th>내용</th><th>결과</th></tr></thead><tbody>
   ${LOGS.map(l=>`<tr style="cursor:pointer" onclick="openDetail(${l.id})"><td style="color:var(--mut2)">${l.t}</td><td><b>${l.nm}</b></td><td>${l.type}</td>
     <td>${({paid:'<span class="pos">정산내용 확정</span>',contacted:'<span style="color:#2b5fd0">직접 연락</span>',hold:'<span style="color:var(--amber)">보류</span>'})[l.result]||l.result}</td></tr>`).join('')}
   </tbody></table>
   <div style="margin-top:16px;background:#fff;border:1px solid var(--line);border-radius:6px;padding:14px;font-size:12.5px;color:var(--mut)">
     처리 요약 — 정산내용 확정 ${LOGS.filter(l=>l.result==='paid').length} · 직접 연락 ${LOGS.filter(l=>l.result==='contacted').length} · 보류 ${LOGS.filter(l=>l.result==='hold').length}
   </div>`;
}
